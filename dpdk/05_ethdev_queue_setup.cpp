// Exercise 05: DPDK ethdev queue setup
//
// Goal: after rte_eth_dev_configure(), create a packet mbuf pool and set up
// RX/TX queues with rte_eth_rx_queue_setup() and rte_eth_tx_queue_setup().
//
// This sample still does not call rte_eth_dev_start(), does not poll packets,
// and does not transmit. It stops at the point where the port has configured
// queues backed by DPDK memory.
...
static void usage(const char *argv0) {
  std::println(
      ...
      "  --yes               Actually configure the port and set up queues\n\n"
      "Examples:\n"
      ...
      "  queues, then closes the port and frees the mempool. It does not call\n"
      "  rte_eth_dev_start().",
...
static std::string make_pool_name(std::uint16_t port_id) {
  return "sample05_p" + std::to_string(port_id) + "_" +
         std::to_string(static_cast<long long>(getpid()));
}

static void print_setup_plan(const Options &options, const rte_eth_conf &conf,
                             int socket_id) {
  std::println("Requested configure and queue setup plan:");
  ...
  std::println("  action: {}",
               options.yes ? "configure and set up queues" : "dry run only");
}
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::println("DPDK ethdev queue setup sample");
    ...
    print_setup_plan(options, conf, socket_id);
    ...
    if (!options.yes) {
      ...
      std::println("Would close the port and free the mempool before exit");
      std::println("Pass --yes to perform these setup calls.");
    } else {
      ...
      print_mempool_summary("Mempool after RX queue setup", pool);
      ...
      print_queue_state("After queue setup", after);
      std::println("Did not call rte_eth_dev_start(), poll RX, or transmit.");
    }

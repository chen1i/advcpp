// Exercise 16: Program virtio queue addresses through VFIO
//
// Goal: write one virtqueue's queue_size, queue_desc, queue_avail, and
// queue_used registers after minimal feature negotiation.
//
// Background
// ----------
// Exercise 15 allocated and VFIO-mapped memory that could hold a split
// virtqueue, but it deliberately did not tell the device about that memory.
// This sample performs the next register step:
//
...
//
// It still does not enable the queue, notify the device, set DRIVER_OK, or
// start DMA.  Before unmapping the vring memory, it resets the device again so
// the device does not keep queue registers pointing at unmapped userspace
// memory.
//
// New concepts
// ------------
// - Programming modern virtio queue address registers
// - Keeping DMA memory mapped while queue registers reference it
...
static void dry_run(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::println("Would read the queue registers back for verification");
  std::println("Would restore queue_select, reset the device, then unmap DMA");
  std::println("Would not write queue_enable, notify, or DRIVER_OK.");
...
static void run_queue_program(const std::string &bdf, const fs::path &dev_dir,
                              const Options &options) {
  require_vfio_driver(dev_dir);
  ...
  QueueView after = read_queue_view(common_mapping, mapping_delta);
  print_queue_view("Selected queue after programming", after);
  ...
  std::println("Did not write queue_enable, notify, or DRIVER_OK.");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (options.yes)
      run_queue_program(bdf, dev_dir, options);
    else

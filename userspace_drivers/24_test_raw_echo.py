...
def wait_for_echo(sock, src_mac, dst_mac, ethertype, timeout, dump_bytes, verbose):
    ...
    while True:
        ...
        ptype = packet_type_name(packet_type)
        print(f"received echo on {addr[0]} pkttype={ptype}")
...
def main():
    ...
    if wait_for_echo(
        sock, src_mac, dst_mac, ethertype, args.timeout, args.dump_bytes, args.verbose
    ):
        return 0

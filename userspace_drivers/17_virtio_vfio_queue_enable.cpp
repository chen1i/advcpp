// Exercise 17: Enable one virtio queue through VFIO
//
// Goal: write one virtqueue's queue registers and set queue_enable=1, while
// still stopping before DRIVER_OK.
//
// Background
// ----------
// Exercise 16 wrote queue_size, queue_desc, queue_avail, and queue_used, then
// stopped before enabling the queue.  This sample performs the next register
// step:
//
...
//
// It still does not notify the device, set DRIVER_OK, or start DMA.  Before
// unmapping the vring memory, it resets the device again so the device does not
// keep an enabled queue pointing at unmapped userspace memory.
//
// New concepts
// ------------
// - queue_enable as the final queue setup bit before DRIVER_OK
...
static void dry_run(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::println("Would not notify or write DRIVER_OK.");
...
static void run_queue_enable(const std::string &bdf, const fs::path &dev_dir,
                             const Options &options) {
  require_vfio_driver(dev_dir);
  ...
  std::println("Did not notify or write DRIVER_OK.");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (options.yes)
      run_queue_enable(bdf, dev_dir, options);
    else

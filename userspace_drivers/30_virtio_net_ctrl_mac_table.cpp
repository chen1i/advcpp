// Exercise 30: Set virtio-net MAC filter table through the control virtqueue
//
// Goal: send VIRTIO_NET_CTRL_MAC_TABLE_SET through the control virtqueue, keep
// the device running, and observe whether frames sent to an extra unicast
// filter MAC are returned through the RX used ring.
//
// Background
// ----------
// Exercise 29 changed the device's primary MAC address through
// MAC_ADDR_SET.  This sample uses the other MAC control command:
//
//   class = VIRTIO_NET_CTRL_MAC
//   cmd   = VIRTIO_NET_CTRL_MAC_TABLE_SET
//   data  = unicast table + multicast table
//   ack   = writable ACK byte
//
// The table command requires VIRTIO_NET_F_CTRL_RX.  The sample resets the
// device on exit, so the filter table is not left behind.
//
// New concepts
// ------------
// - Encoding the virtio-net unicast and multicast MAC tables
// - Sending a control command with two readable data descriptors
// - Testing whether the device accepts packets for an extra filter MAC address
...
constexpr std::array<std::uint8_t, 6> kDefaultFilterMac{
    0x02, 0x00, 0x00, 0x00, 0x30, 0x01};
...
struct Options {
  ...
  std::array<std::uint8_t, 6> filter_mac = kDefaultFilterMac;
...
static void usage(const char *argv0) {
  std::println(
      ...
      "Examples:\n"
      "  {} c1:00.6\n"
      "  {} c1:00.6 --filter-mac 02:00:00:00:30:01 --wait-ms 60000 --yes\n"
      "  {} c1:00.6 --filter-mac 02:00:00:00:30:02 --accept-any-ethertype --yes",
...
static void dry_run(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::println("filter MAC: {}", mac_string(options.filter_mac));
  ...
  if (options.queue_size) {
    if (*options.queue_size < 4) {
      throw std::runtime_error(
          "queue-size must be at least 4 for MAC_TABLE_SET");
    }
    CtrlqDmaLayout layout = compute_ctrlq_dma_layout(
        *options.queue_size, *options.queue_size, *options.queue_size,
        ...
        mac_table_control_data_size(1, 0));
  ...
  std::println("Would publish RX buffers and a MAC_TABLE_SET control command");
  ...
  std::println("Would print whether each RX packet's destination MAC matches "
               "the filter MAC");
  std::println("Would reset the device before exit, so the filter table does "
               "not persist");
  std::println("Pass --yes to perform these device and DMA writes.");
}

static void run_mac_table_sample(const std::string &bdf,
  ...
  std::println("RX buffers: {} x {} bytes", options.rx_buffers,
               options.rx_buffer_size);
  std::println("control command: MAC_TABLE_SET unicast={}",
               mac_string(options.filter_mac));
  std::println("wait-ms: {}", options.wait_ms);
  std::println("stop-after: {} matching RX packets", options.stop_after);
  std::println("match ethertype: {}",
               options.match_ethertype
  ...
  print_virtio_net_config("Initial virtio-net device config", initial_config,
                          guest_features);
  std::println("Unicast filter MAC requested through control queue: {}",
               mac_string(options.filter_mac));
  ...
  publish_mac_table_control_command(ctrl_queue_base, buffer.data(), layout,
                                    options.iova, options.filter_mac);
  std::println("Published control descriptor chain:");
  std::println("  desc 0: control header class={} cmd={}", kVirtioNetCtrlMac,
               kVirtioNetCtrlMacTableSet);
  std::println("  desc 1: unicast table count=1 mac={}",
               mac_string(options.filter_mac));
  std::println("  desc 2: multicast table count=0");
  std::println("  desc 3: writable ACK byte initialized to 0x{:02x}",
  ...
  VirtioNetConfigView after_ctrl_config = read_virtio_net_config(
      device_cfg_mapping, device_cfg_delta, device_cfg.length);
  print_virtio_net_config("Device config after MAC_TABLE_SET",
                          after_ctrl_config, guest_features);

  std::println("MAC_TABLE_SET accepted; observing RX for up to {} ms",
               options.wait_ms);
  std::println("Config MAC: {}", mac_string(initial_config.mac));
  std::println("Unicast filter MAC: {}", mac_string(options.filter_mac));
  ...
  while (matched < options.stop_after &&
    ...
    for (std::uint16_t i = 0; i < count; ++i) {
      ...
      bool dst_is_filter_mac = dst == options.filter_mac;
      ...
      std::println("  dst matches config MAC: {}",
                   dst_is_original_mac ? "yes" : "no");
      std::println("  dst matches filter MAC: {}",
                   dst_is_filter_mac ? "yes" : "no");
      if (dst_is_filter_mac) {
        std::println("  this is the expected MAC_TABLE_SET-path observation");
      }
  ...
  std::println("MAC table observe summary:");
  ...
  std::println("MAC table control sample cleaned up the device.");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (int i = 2; i < argc; ++i) {
      ...
      if (arg == "--filter-mac" && i + 1 < argc) {
        options.filter_mac = parse_mac(argv[++i], "filter-mac");
    ...
    if (options.yes)
      run_mac_table_sample(bdf, dev_dir, options);
    else

// Exercise 20: Set virtio DRIVER_OK through VFIO
//
// Goal: bring a modern virtio device to DRIVER_OK with one empty enabled
// virtqueue, then reset it.
//
// Background
// ----------
// Exercise 17 enabled one queue but stopped before DRIVER_OK.  This sample
// performs the next status step:
//
...
//   write DRIVER_OK
//
// It does not publish any available descriptors and it does not notify the
// device.  The queue is valid but empty.  Before unmapping the vring memory,
// this sample resets the device again so the device does not keep an enabled
// queue pointing at unmapped userspace memory.
//
// Gotcha: a virtio-net device may set NEEDS_RESET shortly after DRIVER_OK in
// this sample.  That is a useful boundary signal, not a VFIO mmap failure: the
// sample intentionally enables only one empty queue and does not provide RX/TX
// buffers or notify work.
//
// New concepts
// ------------
// - DRIVER_OK as the status bit that lets the device run
// - Keeping DMA memory mapped while DRIVER_OK is set
...
static void usage(const char *argv0) {
  std::println(std::cerr,
               ...
               "  {} c1:00.6 --queue 0\n"
               "  {} c1:00.6 --queue 0 --yes\n"
               "  {} c1:00.6 --queue 0 --queue-size 128 --iova 0x200000000 --wait-ms 500 --yes",
...
static void dry_run(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::println("Would write DRIVER_OK and wait {} ms", options.wait_ms);
  std::println("Would restore queue_select, reset the device, then unmap DMA");
  std::println("Would not publish descriptors or notify the queue.");
...
static void run_driver_ok(const std::string &bdf, const fs::path &dev_dir,
                          const Options &options) {
  require_vfio_driver(dev_dir);
  ...
  std::println("Did not publish descriptors or notify the queue.");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (options.yes)
      run_driver_ok(bdf, dev_dir, options);
    else

// Exercise 18: Virtio notify information through VFIO
//
// Goal: compute the MMIO location a future driver would write to notify a
// virtqueue.
//
// Background
// ----------
// Modern virtio PCI devices expose notifications through a NOTIFY_CFG
// capability.  COMMON_CFG gives each queue a queue_notify_off value, and the
// NOTIFY_CFG capability gives the multiplier:
//
//   notify BAR offset = notify_cfg.offset
//                     + queue_notify_off * notify_off_multiplier
//
// This sample parses NOTIFY_CFG and reads queue_notify_off for one queue.  It
// does not write the notify register, enable queues, set DRIVER_OK, or start
// DMA.
//
// New concepts
// ------------
// - NOTIFY_CFG capability
// - notify_off_multiplier
// - queue_notify_off as a selected-queue field
// - Computing the future notify MMIO file offset
...
struct QueueNotifyView {
  std::uint16_t selected = 0;
  std::uint16_t queue_size = 0;
  std::uint16_t queue_enable = 0;
  std::uint16_t queue_notify_off = 0;
  std::optional<std::uint16_t> queue_notify_data;
};
...
static void usage(const char *argv0) {
  std::println(std::cerr,
               ...
               "Defaults:\n"
               "  queue = current selected queue\n"
               "  default mode = dry-run\n\n"
               "Examples:\n"
               "  {} c1:00.6 --show\n"
               "  {} c1:00.6 --show --queue 0 --yes\n"
               "  {} c1:00.6 --show --queue 1 --yes",
...
static QueueNotifyView read_queue_notify_view(const MappedRegion &mapping,
                                              std::size_t mapping_delta) {
  QueueNotifyView view;
  view.selected =
      mapping.read16(mapping_delta + VIRTIO_PCI_COMMON_Q_SELECT);
  view.queue_size =
      mapping.read16(mapping_delta + VIRTIO_PCI_COMMON_Q_SIZE);
  view.queue_enable =
      mapping.read16(mapping_delta + VIRTIO_PCI_COMMON_Q_ENABLE);
  view.queue_notify_off =
      mapping.read16(mapping_delta + VIRTIO_PCI_COMMON_Q_NOFF);
#ifdef VIRTIO_PCI_COMMON_Q_NDATA
  view.queue_notify_data =
      mapping.read16(mapping_delta + VIRTIO_PCI_COMMON_Q_NDATA);
#endif
  return view;
}

static void print_queue_notify_view(std::string_view label,
                                    const QueueNotifyView &view) {
  std::println("{}:", label);
  std::println("  selected: {}", view.selected);
  std::println("  queue_size: {}", view.queue_size);
  std::println("  queue_enable: {}", view.queue_enable);
  std::println("  queue_notify_off: {}", view.queue_notify_off);
  if (view.queue_notify_data) {
    std::println("  queue_notify_data: {}", *view.queue_notify_data);
  } else {
    std::println("  queue_notify_data: <not exposed by this kernel header>");
  }
}

static std::uint64_t notify_bar_offset(const VirtioCap &notify,
                                       std::uint16_t queue_notify_off) {
  return notify.offset +
         static_cast<std::uint64_t>(queue_notify_off) *
             notify.notify_off_multiplier;
}

static void print_notify_result(const VirtioCap &notify,
                                const vfio_region_info &bar_region,
                                const QueueNotifyView &queue) {
  std::uint64_t bar_offset =
      notify_bar_offset(notify, queue.queue_notify_off);
  std::uint64_t vfio_file_offset = bar_region.offset + bar_offset;

  std::println("Notify calculation:");
  std::println("  notify_cfg.offset: 0x{:x}", notify.offset);
  std::println("  queue_notify_off: {}", queue.queue_notify_off);
  std::println("  notify_off_multiplier: {}",
               notify.notify_off_multiplier);
  std::println("  BAR-relative notify offset: 0x{:x}", bar_offset);
  std::println("  VFIO device-fd file offset: 0x{:x}", vfio_file_offset);
  std::println("  future notify value without VIRTIO_F_NOTIFICATION_DATA: {}",
               queue.selected);
  if (queue.queue_notify_data) {
    std::println("  future notify value with VIRTIO_F_NOTIFICATION_DATA: {}",
                 *queue.queue_notify_data);
  }

  if (bar_offset + sizeof(std::uint16_t) > notify.offset + notify.length) {
    std::println("  warning: computed notify offset is outside NOTIFY_CFG "
                 "length");
  }
}
...
static void show_notify_info(const std::string &bdf, const fs::path &dev_dir,
  ...
  __u32 notify_region_index = vfio_bar_region_index(notify.bar);
  RegionInfo notify_region =
      get_region_info(vfio.device.get(), notify_region_index);
  const vfio_region_info &notify_info = notify_region.info;
  ...
  if (notify.length == 0)
    throw std::runtime_error("NOTIFY_CFG length is zero");
  if (notify.offset >= notify_info.size)
    throw std::runtime_error("NOTIFY_CFG offset is past BAR region size");
  ...
  std::println("  notify_off_multiplier: {}",
               notify.notify_off_multiplier);
  std::println("NOTIFY BAR{} region:", notify.bar);
  std::println("  size: 0x{:x} ({})", notify_info.size, notify_info.size);
  std::println("  VFIO region offset: 0x{:x}", notify_info.offset);
  ...
  QueueNotifyView current = read_queue_notify_view(mapping, mapping_delta);
  print_queue_notify_view("Current selected queue notify view", current);
  print_notify_result(notify, notify_info, current);

  if (requested_queue && !yes) {
    std::println("Would write queue_select={} and read queue_notify_off",
                 *requested_queue);
    std::println("Pass --yes to perform this selector write.");
    return;
  }
  ...
  try {
    write_queue_select(mapping, mapping_delta, *requested_queue);
    changed_selection = true;
    QueueNotifyView selected = read_queue_notify_view(mapping, mapping_delta);
    print_queue_notify_view("Requested queue notify view", selected);
    print_notify_result(notify, notify_info, selected);
  } catch (...) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    show_notify_info(bdf, dev_dir, queue, yes);
    return 0;

// Exercise 15: Virtio split vring DMA layout through VFIO
//
// Goal: allocate and VFIO-map memory that could hold one split virtqueue.
//
// Background
// ----------
// After FEATURES_OK, a virtio driver must prepare virtqueue memory before it
// can set DRIVER_OK.  A split virtqueue has three logical areas:
//
//   descriptor table -> available ring -> used ring
//
// Modern virtio PCI exposes separate queue_desc, queue_avail, and queue_used
// registers, but it is still convenient for a tutorial driver to allocate one
// contiguous userspace buffer and hand three IOVA addresses to the device.
//
// This sample only prepares and maps that memory.  It writes queue_select so it
// can read the target queue's maximum size, then restores queue_select before
// exit.  It does not write queue_desc/queue_avail/queue_used, enable a queue,
// notify the device, set DRIVER_OK, or start DMA.
//
// New concepts
// ------------
// - split virtqueue descriptor/available/used layout
// - vring size and alignment
// - IOVA addresses for queue_desc / queue_avail / queue_used
// - mapping queue memory with VFIO_IOMMU_MAP_DMA
...
// linux/virtio_ring.h contains C inline helpers that are not C++-clean on all
// systems.  The split ring sizes used here are fixed by the virtio
// specification, so keep the small constants local.
...
static MappedRegion map_common_cfg(int device_fd, const VirtioCap &common,
  ...
  std::size_t inspect_length =
      std::min<std::size_t>(common_length, VIRTIO_PCI_COMMON_Q_USEDHI + 4);
...
static void print_vring_layout(const VringLayout &layout,
  ...
  std::println("  descriptor table:");
  std::println("    offset: 0x{:x}", layout.desc_offset);
  std::println("    size: {}", layout.desc_size);
  std::println("    IOVA for queue_desc: 0x{:x}",
               base_iova + layout.desc_offset);
  std::println("  available ring:");
  std::println("    offset: 0x{:x}", layout.avail_offset);
  std::println("    size: {}", layout.avail_size);
  std::println("    IOVA for queue_avail: 0x{:x}",
               base_iova + layout.avail_offset);
  std::println("  used ring:");
  std::println("    offset: 0x{:x}", layout.used_offset);
  std::println("    size: {}", layout.used_size);
  std::println("    IOVA for queue_used: 0x{:x}",
               base_iova + layout.used_offset);
...
static void dry_run(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::println("Would mmap virtio COMMON_CFG");
  std::println("Would write queue_select={} and read queue_size", options.queue);
  std::println("Would allocate a zeroed userspace vring buffer");
  std::println("Would call VFIO_IOMMU_MAP_DMA with READ|WRITE permissions");
  std::println("Would call VFIO_IOMMU_UNMAP_DMA before exit");
  std::println("Would restore the original queue_select before exit");
  std::println("Would not write queue_desc/queue_avail/queue_used or enable "
               "the queue.");
  std::println("Pass --yes to perform the selector write and DMA map/unmap.");
}
...
static void map_vring_for_queue(const std::string &bdf,
                                const fs::path &dev_dir,
                                const Options &options) {
  require_vfio_driver(dev_dir);
  ...
  print_status("device_status",
               common_mapping.read8(mapping_delta + VIRTIO_PCI_COMMON_STATUS));
  ...
  QueueView queue = read_queue_view(common_mapping, mapping_delta);
  print_queue_view("Selected queue before DMA map", queue);
  ...
  std::uint16_t chosen_queue_size =
      choose_queue_size(queue.size, options.queue_size);
  if (chosen_queue_size != queue.size) {
    std::println("using reduced queue_size: {} (device max {})",
                 chosen_queue_size, queue.size);
  }
  ...
  std::println("Did not write queue_desc/queue_avail/queue_used, queue_enable, "
               "notify, or DRIVER_OK.");
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (!is_power_of_two(options.align) || options.align < kVringUsedAlignSize) {
      throw std::runtime_error(
          "align must be a power of two and at least VRING_USED_ALIGN_SIZE");
    }

    if (options.yes)
      map_vring_for_queue(bdf, dev_dir, options);
    else

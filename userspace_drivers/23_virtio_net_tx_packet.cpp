...
constexpr std::uint16_t kDefaultRxBuffers = 32;
constexpr std::size_t kDefaultRxBufferSize = 2048;
constexpr std::size_t kDefaultDumpBytes = 96;
...
static std::vector<std::uint8_t> parse_hex_bytes(std::string_view text,
                                                 std::string_view name) {
  std::string hex;
...
static void print_dma_layout(const DmaLayout &layout, std::uint64_t base_iova,
  ...
  std::println("  TX packet bytes: {}", layout.tx_packet_size);
...
static void write_be16_to_bytes(std::uint8_t *bytes, std::uint16_t value) {
  bytes[0] = static_cast<std::uint8_t>(value >> 8);
  bytes[1] = static_cast<std::uint8_t>(value & 0xffu);
}

static std::vector<std::uint8_t> default_payload() {
  std::vector<std::uint8_t> payload;
...
static std::array<std::uint8_t, 6> broadcast_mac() {
  return {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
}

static std::size_t padded_ethernet_payload_size(std::size_t payload_size) {
  std::size_t min_payload = kEthernetMinFrameSize - kEthernetHeaderSize;
  return std::max(payload_size, min_payload);
}

static std::size_t tx_packet_size_for_payload(std::size_t payload_size) {
  return kVirtioNetHeaderSize + kEthernetHeaderSize +
         padded_ethernet_payload_size(payload_size);
}

static void build_tx_packet(std::uint8_t *packet,
...
static void dump_rx_used_buffers(std::uint8_t *dma_base,
...
static void dry_run(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::size_t payload_size =
      options.payload.empty() ? default_payload().size() : options.payload.size();
...
static void run_tx_packet(const std::string &bdf, const fs::path &dev_dir,
  ...
  std::vector<std::uint8_t> payload =
      options.payload.empty() ? default_payload() : options.payload;
